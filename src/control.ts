...
export async function updateRuntimeOrder(profileDir: string, order: string[]): Promise<boolean> {
  const path = addressFile(profileDir)
  ...
  try {
    response = await fetch(`${address.url}/dsh-harmony/order`, {
      method: 'POST',
    ...
    if (error instanceof Error && error.name === 'TimeoutError') {
      throw new Error('dsh-harmony: running Harness did not finish the order update within 30 seconds')
    }
    return false
  }
  if (response.status === 404) return false
  const body = await response.json() as { dir?: string; order?: string[]; error?: string }
  if (!response.ok) throw new Error(body.error ?? `dsh-harmony: runtime returned HTTP ${response.status}`)
  if (body.dir !== profileDir || body.order?.length !== order.length || body.order.some((name, index) => name !== order[index])) {
    throw new Error('dsh-harmony: runtime response does not match the requested profile order')
  }
  return true
}
