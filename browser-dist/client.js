...
`;
        ...
        function RuntimePrompt({ t }) {
            ...
            useEffect(() => { if (status?.state === 'missing' || status?.state === 'desktop-inactive')
            ...
            if (dismissed || status === null || status.state === 'active' || status.state === 'ignored' || status.state === 'removed')
                return null;
            const installed = status.state === 'installed';
            const desktopInactive = status.state === 'desktop-inactive';
            return h('div', { className: 'dshHarmonyRuntimeLayer', role: 'presentation' }, h('section', { className: 'dshHarmonyRuntimeDialog', role: 'alertdialog', 'aria-modal': 'true', 'aria-labelledby': 'dsh-harmony-runtime-title' }, h('h2', { id: 'dsh-harmony-runtime-title' }, t(desktopInactive ? 'runtimeDesktopTitle' : 'runtimeTitle')), h('p', null, t(desktopInactive ? 'runtimeDesktopBody' : installed ? 'runtimeInstalled' : status.state === 'working' ? 'runtimeWorking' : 'runtimeBody')), status.error ? h('p', { className: 'dshHarmonyRuntimeError', role: 'alert' }, `${t('runtimeError')}: ${status.error}`) : null, h('div', { className: 'dshHarmonyRuntimeActions' }, installed
        ...
        function PatchStatusPage({ t }) {
            ...
            return h('section', { className: 'dshHarmonyPatchPage' }, h('header', null, h('h2', { className: 'dshHarmonyHeading' }, t('patchTitle')), h('p', { className: 'dshHarmonyIntro' }, t('patchIntro'))), loading ? h('div', { className: 'dshHarmonySkeleton', 'aria-label': t('loading') }, h('div'), h('div')) :
                patches.length === 0 ? h('p', { className: 'dshHarmonyStatus' }, t('patchEmpty')) :
                    h('div', { className: 'dshHarmonyPatchWorkspace' }, h('ul', { className: 'dshHarmonyPatchList', role: 'listbox', 'aria-label': t('patchTitle') }, patches.map(item => h('li', { key: item.key }, h('button', {
                        className: 'dshHarmonyPatchRow', type: 'button', role: 'option',
                        'aria-selected': patch?.key === item.key,
                        onClick: () => setSelected(item.key),
                    }, h('span', { className: 'dshHarmonyPatchState', 'data-state': item.state, title: stateLabel(item.state) }), h('span', null, h('span', { className: 'dshHarmonyPatchKey', title: item.key }, item.key), h('span', { className: 'dshHarmonyPatchTarget' }, `${item.target.package}/${item.file ?? item.target.files.join(' | ')} · ${stateLabel(item.state)}`)))))), patch === undefined ? h('p', { className: 'dshHarmonyStatus' }, t('patchSelect')) :
                        h('article', { className: 'dshHarmonyPatchDetail' }, h('div', { className: 'dshHarmonyPatchHeader' }, h('div', null, h('h3', { className: 'dshHarmonyTitle' }, patch.key), h('p', { className: 'dshHarmonyScope' }, `${stateLabel(patch.state)} · ${patch.kind}${patch.operation ? ` / ${patch.operation}` : ''}`)), h('div', { className: 'dshHarmonyPatchChain' }, h('button', {
                            className: 'dshHarmonySecondary', type: 'button', disabled: busy === patch.key,
                            onClick: () => { void toggle(true); },
                        }, t(patches.filter(item => item.owner === patch.owner).every(item => item.state === 'disabled') ? 'enableProvider' : 'disableProvider')), h('button', {
                        ...
                        }, t(patch.state === 'disabled' ? 'enable' : 'disable')))), h('div', { className: 'dshHarmonyFacts' }, h('span', null, `${t('patchTarget')}: ${patch.target.package}`), patch.target.version ? h('span', null, `${t('patchVersion')}: ${patch.target.version}`) : null, h('span', null, `${t('patchFile')}: ${patch.file ?? patch.target.files.join(' | ')}`), h('span', null, `${t('patchLoaded')}: ${patch.loaded ? '✓' : '—'}`), h('span', null, `${t('patchMatches')}: ${patch.matches}`), h('span', null, `${t('patchGeneration')}: ${patch.generation}`), patch.operation ? h('span', null, `${t('patchOperation')}: ${patch.operation}`) : null), patch.error ? h('p', { className: 'dshHarmonyConstraint dshHarmonyError', role: 'alert' }, patch.error) : null, inspection ? h(React.Fragment, null, h('h4', { className: 'dshHarmonyScope' }, t('patchChain')), h('div', { className: 'dshHarmonyPatchChain' }, inspection.steps.map(step => h('span', { key: step.key }, `${step.key} · ${step.matches}`))), h('h4', { className: 'dshHarmonyScope' }, t('patchOriginal')), h('pre', { className: 'dshHarmonyPatchCode' }, inspection.original), inspection.steps.map(step => h(React.Fragment, { key: step.key }, h('h4', { className: 'dshHarmonyScope' }, `${t('patchIntermediate')}: ${step.key}`), h('pre', { className: 'dshHarmonyPatchCode' }, step.source))), h('h4', { className: 'dshHarmonyScope' }, t('patchFinal')), h('pre', { className: 'dshHarmonyPatchCode' }, inspection.final)) : null)), error ? h('p', { className: 'dshHarmonyHint dshHarmonyError', role: 'alert' }, `${t('runtimeError')}: ${error}`) : null);
        }
        function HarmonySettings({ t }) {
            ...
            useEffect(() => { void load(); }, []);
            useEffect(() => { if (closePrompt !== null)
                promptButton.current?.focus(); }, [closePrompt]);
            useEffect(() => {
                const guard = () => {
                    if (!dirtyRef.current)
            ...
            const moveTo = (name, target) => {
                ...
                setDraftOrder(current => {
                    ...
                    if (name === harmonyPlugin || from === -1 || target >= current.length || from === target)
                        return current;
                    const next = [...current];
                    next.splice(from, 1);
            ...
            return h('div', { className: 'dshHarmonyPage' }, h('nav', { className: 'dshHarmonyTabs', role: 'tablist', 'aria-label': t('nav') }, h('button', {
            ...
            }, t('patchPage'))), page === 'patches' ? h(PatchStatusPage, { t }) : h(React.Fragment, null, h('header', null, h('h2', { className: 'dshHarmonyHeading' }, t('title')), h('p', { className: 'dshHarmonyIntro' }, t('intro'))), view.incompatibilities.length > 0
                ? h('p', { className: 'dshHarmonyWarning', role: 'status' }, `${t('incompatibilityWarning')} ${view.incompatibilities
                    .map(item => `${item.declaredBy} ↔ ${item.conflictsWith}`)
                    .join(' · ')}`)
                : null, h('div', { className: 'dshHarmonyWorkspace' }, h('ul', {
                ref: listRef,
                ...
                return h('li', { key: name }, h('button', {
                    ...
                    onPointerDown: (event) => {
                        if (event.button !== 0 || isFixed || saving)
                            return;
                    ...
                    onKeyDown: (event) => {
                        ...
                        if (event.altKey)
                            ...
                            selectBy(name, offset);
                    },
                }, h('span', { className: 'dshHarmonyGrip', 'aria-hidden': 'true' }, isFixed ? '•' : '⠿'), h('span', { className: 'dshHarmonyIndex' }, String(index + 1).padStart(2, '0')), h('span', { className: 'dshHarmonyName', title: name }, listName(name)), isFixed
