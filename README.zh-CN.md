...
`element()` 修改选中的 `jsx` / `jsxs` 调用点；`component()` 修改所有通过同一已初始化变量或具名函数声明进行的调用。函数声明会被改写为 `const` 以组合 Component Patch，因此生成绑定不具有声明提升。参见 [React 集成](https://memorax-ai.github.io/dsh-harmony/zh/integrations/react)。
