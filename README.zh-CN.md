...
`element()` 修改选中的 `jsx` / `jsxs` 调用点，`component()` 修改所有通过同一已初始化变量或具名函数声明进行的调用。Harmony 会把函数声明改写为 `const`，让后续 Component Patch 可以继续修改它；新绑定不再提升。参见 [React 集成](https://memorax-ai.github.io/dsh-harmony/zh/integrations/react)。
